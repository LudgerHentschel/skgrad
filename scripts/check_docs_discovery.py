...
BASE = "https://ludgerhentschel.github.io/skgrad/"
...
def main():
    ...
    for name in ("index", "getting-started", "models", "api", "semantics"):
        assert BASE + name + ".html" in locations, f"Guide omitted from sitemap: {name}"
    api = (build / "api.html").read_text()
    assert 'id="skgrad.input_gradient"' in api, "API reference was not expanded"
    quickstart = (build / "getting-started.html").read_text()
    assert "input_gradient" in quickstart and "LogisticRegression" in quickstart, "Quickstart was not expanded"
